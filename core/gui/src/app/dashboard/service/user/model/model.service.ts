...
export class ModelService {
   ...
   * @param did Model ID
   * @param dvid (Optional) Model version ID. If omitted, the latest version is downloaded.
  ...
  public retrieveModelVersionZip(did: number, dvid?: number): Observable<Blob> {
    ...
    return this.http.get(`${AppSettings.getApiEndpoint()}/model/${did}/versionZip`, {
      params,
      responseType: "blob",
    });
  }
  ...
  public createModelVersion(did: number, newVersion: string): Observable<ModelVersion> {
    return this.http
      ...
      }>(`${AppSettings.getApiEndpoint()}/${MODEL_BASE_URL}/${did}/version/create`, newVersion, {
  ...
  public multipartUpload(
    ...
    return new Observable(observer => {
      ...
      const subscription = this.initiateMultipartUpload(modelName, filePath, partCount)
        .pipe(
          switchMap(initiateResponse => {
            ...
            return from(presignedUrls).pipe(
              ...
              switchMap(() =>
                this.finalizeMultipartUpload(modelName, filePath, uploadId, uploadedParts, physicalAddress, false)
              ),
              tap(() => {
  ...
  private initiateMultipartUpload(
    ...
    return this.http.post<{ uploadId: string; presignedUrls: string[]; physicalAddress: string }>(
      `${AppSettings.getApiEndpoint()}/${MODEL_BASE_URL}/multipart-upload`,
      {},
      { params }
    );
  }
   ...
   */
  public resetModelFileDiff(did: number, filePath: string): Observable<Response> {
    const params = new HttpParams().set("filePath", encodeURIComponent(filePath));

    return this.http.put<Response>(`${AppSettings.getApiEndpoint()}/${MODEL_BASE_URL}/${did}/diff`, {}, { params });
  }
  ...
  public deleteModelFile(did: number, filePath: string): Observable<Response> {
    ...
    return this.http.delete<Response>(`${AppSettings.getApiEndpoint()}/${MODEL_BASE_URL}/${did}/file`, { params });
  }
  ...
  public getModelDiff(did: number): Observable<ModelStagedObject[]> {
    return this.http.get<ModelStagedObject[]>(`${AppSettings.getApiEndpoint()}/${MODEL_BASE_URL}/${did}/diff`);
  }

  /**
   * retrieve a list of versions of a model. The list is sorted so that the latest versions are at front.
   * @param did
   * @param isLogin
   */
