...
def debug_workflow(workflow_name: str) -> None:
    ...
    if paths:
        ...
        print(f"   - path workflow dict keys: {list(path_workflow.workflow_dict.keys())}")
        if "content" in path_workflow.workflow_dict:
            ...
            if "operators" in content:
                ...
                if operators:
                    ...
                    print(f"   - first operator keys: {list(operators[0].keys() if isinstance(operators[0], dict) else [])}")
            ...
            if "links" in content:
                ...
                if links:
                    ...
                    print(f"   - first link keys: {list(links[0].keys() if isinstance(links[0], dict) else [])}")
